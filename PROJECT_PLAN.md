...
- `thesis/7_References.md` — APA 7 reference list, **numbered in citation order** (per
  the Universität Wien template supplied 2026-07-15, whose §7 References carries the
  subheading "Citation of Relevant Literature"), 50 entries: all 33 cited academic
  corpus sources + 5 H&M reports + 4 Inditex reports + EU Strategy, CSRD, Green Claims
  Directive, ACM decision, Fashion Revolution (FTI 2022/2023, WFF 2024) and Changing
  Markets Synthetics Anonymous 2.0. All 33 academic sources confirmed cited in-text
  before inclusion; first-citation order computed by scanning Ch.1–6 in reading order.
...
- `thesis/THESIS_FINAL.md` — single assembled manuscript matching the Universität Wien
  ...
  adapting heading titles to the research (notes on template pp. 4/6), consistent with
  CLAUDE.md §1.
...
styles); insert the Universität Wien logo on the title page and confirm the exact
degree-programme name from the student record sheet; let Word generate the ToC
pagination; write the Acknowledgments and the signed Declaration of Authorship; final
human read-through; and the supervisor check on the presentation-vs-thesis scope
evolution (4-year window, tightened RQ wording).
